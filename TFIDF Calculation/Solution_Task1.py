...
def write_to_file(key, value):
    with open(sys.argv[3], 'a') as text_file:
        text_file.write("%s: %s\n" % (key, value))
...
def print_ID(any):
    ...
    for doc in coll.items():
        totalTerms = len(doc[1])

        doc_sorted = OrderedDict(
            sorted(doc[1].items(), key=lambda x: x[1], reverse=True))
        all_items.update(doc_sorted)

    print("There are %s documents in this data set and contains %d terms." %
          (len(coll.items()), totalTerms))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        ...
        sys.stderr.write(
            "USAGE: %s <coll-file> <stop-words> <write-file>\n" % sys.argv[0])
        sys.exit()
